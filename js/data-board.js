...
window.RAT_RACE = [
  /* ★ 发薪日 = 3 / 24 格，等距分布（2 / 10 / 18，任意相邻两个的间隔恰好 8 格）。
     结算步长：【一个发薪日 = 一年】—— 每经过一个发薪日结算最旧的 1 个未结年份；
     没踩到发薪日的年份会积欠，在后续发薪日逐个结清（突变点 / 终局则一次结清）。
     密度决定「多久发一次薪」与积欠的深浅，搭配实验记录见
     docs/结算步长与发薪日密度.md（方案 A 两次上线两次回滚，玩家读不懂多年合并结算）。
     ⚠️ 等距的副产品：单粒骰子（≤6 步）永远跨不过第 2 个发薪日，
        「一回合跨多个发薪日」只在银翅膀（2 粒骰）时才可能发生。 */
...
/* ---------- 外圈：财务自由圈（掷 2 粒骰子） ---------- */
window.FAST_TRACK = [
  /* ★ 分红日 = 4 / 24 格（原 8 个）。结算步长与内圈同为「一个结算格 = 1 年」，
     密度 > 1 → settledAge 追平年龄后，多余的分红日记「本年已结」略过 ——
     这是外圈的常态，与内圈的「积欠」互为镜像。
