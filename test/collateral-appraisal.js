...
assert.equal(cp.appraisal.value,17710);
assert.equal(cp.collateral,8855);
assert.equal(cp.appraisal.rows.length,1);
assert.equal(cp.appraisal.rows[0].item.symbol,'TEST');
assert.equal(cp.appraisal.rows[0].collateral,8855);
...
assert.equal(row.value,Math.round(236500*E.marketIndex(g,'realEstate')));
assert.equal(row.collateral,Math.round(row.value*0.45));
...
assert.equal(joint.collateral,Math.round(joint.value*joint.item.dp/joint.book));
