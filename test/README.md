...
守住「一轮 = 一年、一个结算年 = 12 个月」这条时间主轴：口径真源唯一
...
⑥ 一整局的结算区间是否零重叠、全覆盖（即「有没有一年被结两次 / 漏掉」）？**

```bash
node test/payday-trigger.js      # 87 项断言 + 20 条已知偏差提示，退出码 0 = 全通过
```
...
第 ⑯ 节用**属性拦截器**（`Object.defineProperty` 拦截 `settledAge` 的每一次写入）覆盖
**全部四个结算入口**（movePlayer / settleAtBreak / finalSettle / retireBreakOf），
断言：结算区间**零重叠**、**无一年被漏掉**、**无一年被结两次**、**无静默跳年**、
每个发薪日**恰好一次**结算、每笔金额满足「年结余 × 年数」，
以及日志里不再出现会被读成「N 次」的旧写法 `结算 N 年：入账`。

> ⚠️ 只钩导出的 `E.settleAtBreak` 是**无效的** —— `startJobless` / `retireBreakOf` / `finalSettle`
> 调的都是模块内函数。必须拦截属性写入，否则会误报「突变点 0 笔」。
...
> 后**必须重跑**；改动 `paydayNoticeOf` 的触发条件、**结算文案**、或**新增任何结算入口**
> 后也必须重跑（第 ⑮ / ⑯ 节）。

---

## 7. `payday-settlement.js` —— 结算步长（方案 A）

验证方案 A 的结算步长在所有构型下自洽：

- **步长口径**：一次结算 = 结清「自上次结算以来经过的年数」（`due = ageOf(g) − p.settledAge`）；
  逐格验证每个发薪日单格触发的 due 相同；落后 1 年结 1 年、本年已结（due=0）记略过。
- **入账与摊还**：入账 = `年结余 × due`；摊还 due × 12 期（领几年就还几年）；
  多年摊还途中负债被还清后期数少于上限属正常（断言写区间而非等号）。
- **跨多个结算格**：等距间隔 8 格 > 单粒骰上限 6 步 → 常态最多跨 1 个；
  银翅膀 2 粒骰最多跨 2 个，但真正结算的只有第 1 个，其余如实记「略过」。
- **边界**：跨 0 个 / 本年已结（提示 kind = `already`）/ 入不敷出 / 确认面板「一次结算，覆盖」文案。
- **一整局**：一生结算总年数 > 命中格数（一次覆盖多年是常态）、
  每笔金额满足「年结余 × 年数」、**密度守门**（0.3—0.6 个/轮，标定 0.44）。

```bash
node test/payday-settlement.js   # 51 项断言，退出码 0 = 全通过
```

> 结算步长与发薪日密度的搭配规则与实验记录见
> [`docs/结算步长与发薪日密度.md`](../docs/结算步长与发薪日密度.md)。
> **改动棋盘格型分布后必须重跑本脚本**（第 ⑤ 段是密度的守门人）。
...
起点休假、银翅膀掷 2 粒、公益捐赠税前扣除、健康危机、年龄推进曲线。
