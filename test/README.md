...
## 7. `payday-settlement.js` —— 结算步长（一个结算格 = 一年）

验证「每经过一个结算格恰好结算 1 年」在所有构型下自洽：

- **步长口径**：每个发薪日 / 分红日结算【最旧的 1 个未结年份】（`settledAge + 1`）；
  逐格验证每个发薪日单格触发都只结 1 年、积欠保留；落后 1 年结 1 年、无积欠记略过。
- **入账与摊还**：每笔入账 = `年结余 × 1`、摊还 12 期；摊还途中负债被还清后期数少于上限属正常（断言写区间而非等号）。
- **跨多个结算格**：等距间隔 8 格 > 单粒骰上限 6 步 → 常态最多跨 1 个；
  银翅膀 2 粒骰最多跨 2 个 → 结 2 年（每格各 1 年）；积欠不足时第 2 个记「略过」。
- **边界**：跨 0 个 / 本年已结（提示 kind = `already`）/ 入不敷出 / 确认面板「结算 1 年（第 X 岁）」文案与剩余积欠提示。
- **一整局**：每笔恰好 1 年（不存在「一笔多年」）、走完全程终局结清推平 `settledAge`
  （积欠不丢年）、每笔金额满足「年结余 × 年数」、**密度守门**（0.3—0.6 个/轮，标定 0.44）。
