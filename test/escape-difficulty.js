...
/* ⚠️ 带宽 25%—80% 是【按实测定的】，而不是拍脑袋：
   「一个结算格 = 一年」+ 内圈 3 个等距发薪日（2/10/18）是当前定稿口径，
   同一 AI 在 6 种子下实测 39%（中位 60 岁）—— 收入按「每格 1 年」逐笔到达，
   比方案 A 的「积欠一次结清」更慢，出圈更难，但在带宽内。
   ⚠️ 历史实验（均已回滚，记录见 docs/结算步长与发薪日密度.md）：
      · 方案 A（结清积欠）+ 3 等距发薪日 → 57% / 中位 57 岁 —— 玩家读不懂多年合并结算；
      · 「每格 1 年」+ 7 发薪日 → 71% —— 发薪日太密、结算太频繁；
      · 「每格 1 年」+ 3 发薪日且无积欠兜底 → 覆盖率 44%，几乎不可能出圈。 */
